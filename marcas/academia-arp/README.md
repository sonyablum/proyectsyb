# 🎓 Academia ARP — Academia de Relaciones Públicas

> Status: 🟢 Tienda activa con 3 productos sin promoción · catálogo de cursos por construir.

## Identidad

**Qué es**: la academia digital del grupo. Educación masiva B2C en RRPP 6.0, IA aplicada a comunicación y reputación algorítmica. Productiza el Marco ACA™ y los frameworks (RICFE®, IAPRMIL, IA-Ethics) para profesionales y estudiantes.

**ARP** = **Academia de Relaciones Públicas** ✅ (sigla oficial confirmada).

**Target**:
- Profesionales de PR, comunicación corporativa, marketing digital LATAM y España.
- Direcciones de comunicación PYME que no acceden a Blum Digital PR (precio).
- Estudiantes universitarios de comunicación, periodismo, marketing.

**Diferenciador**: contenido derivado del trabajo académico de Sonia Yánez (Marco ACA™, DOIs Zenodo, RICFE®). Es la **versión escalable** del conocimiento que Blum entrega 1-a-1.

---

## Líneas de producto

### 1. Cursos pregrabados (digital evergreen)
| Categoría | Idea inicial | Estado |
|---|---|---|
| Fundamentos | "Reputación algorítmica para profesionales de PR" | 🟡 borrador idea |
| Herramienta | "RICFE® — prompting estratégico aplicado a comunicación" | 🟡 borrador idea |
| Auditoría | "Aplica el Marco ACA™ a tu marca en 5 días" | 🟡 borrador idea |
| Ética | "IA-Ethics para comunicadores" | 🟡 borrador idea |

### 2. Plantillas y herramientas pregrabadas
- Plantillas de PR digital (notas de prensa IA-friendly, kits de medios, prompts de monitoreo).
- Templates de auditoría ACA™ self-service (versión simplificada).
- Bibliotecas de prompts.

### 3. Tienda / shop — 🟢 ACTIVA (3 productos, cero promoción)
- 3 productos vivos en este momento *(detalle por catalogar en `marcas/academia-arp/tienda.md`)*.
- Sin promoción → palanca obvia: activar plan de difusión.
- Posicionamiento: extensión cultural de la marca, no centro de ingresos.
- *[CONFIRMAR]*: plataforma de la tienda + fulfillment (Printful · Printify · proveedor LATAM).

---

## Modelo de monetización

| Vehículo | Precio sugerido | Estrategia |
|---|---|---|
| Curso individual | $49 – $199 | Catálogo evergreen |
| Bundle de plantillas | $29 – $89 | Upsell post-curso |
| Membresía / suscripción | $19/mes *[CONFIRMAR]* | Acceso continuo + actualizaciones |
| Merch | $20 – $40 | Marca, no margen |

> Todo el pricing arriba es **propuesta de trabajo**, sujeto a tu validación.

---

## Subagentes que la operan

| Subagente | Función en Academia ARP |
|---|---|
| `agente-marketing` | Catálogo, lanzamientos, secuencias email, ads |
| `agente-ventas` | Funnels, abandono de carrito, upsells |
| `agente-automatizaciones` | Plataforma cursos, e-commerce, fulfillment merch |
| `agente-cfo` | Royalties, márgenes, refunds |
| `agente-pr-medios` | PR de lanzamientos en medios sectoriales |

`agente-congresos-academicos` **no toca** Academia ARP (el contenido ya viene de Sonia personal a través de licencias).

---

## Mi papel (Jefe de Despacho)

- Catálogo vivo (idea / en producción / lanzado / archivado).
- Roadmap editorializado de lanzamientos por trimestre.
- Plantillas de página de venta, secuencias email, copys de ads.
- Mantener separación clara: contenido gratuito (lead magnet) vs pago.
- Aviso cuando un curso canibaliza un servicio Blum (debe haber claridad de tier).

---

## Indicadores que vigilo

- Ingresos por curso (monthly)
- Tasa de finalización (engagement educativo)
- LTV por alumno
- CAC alumno (canal a canal)
- Ticket medio del shop
- Refund rate
- Conversión Academia → consulta Blum (cuando aplique)

---

## Estado actual (1-may-2026)

🟢 **Tienda activa con 3 productos** — sin promoción todavía (palanca de crecimiento inmediata).
🟡 **Catálogo de cursos**: sin productos lanzados; pipeline editorial por construir.

Falta:
- Catalogar los 3 productos activos en la tienda (nombres, precios, fotos, URLs).
- Plataforma de cursos seleccionada y configurada.
- Identidad visual unificada.
- Web propia consolidada (si la tienda tiene URL, puede ser punto de entrada inicial).
- Plan de promoción Q2 2026 para los 3 productos vivos.

---

## Pendientes Academia ARP

| # | Tarea | Prioridad |
|---|---|---|
| AC1 | Catalogar los 3 productos activos (nombres, precios, fotos, URLs, fulfillment) en `marcas/academia-arp/tienda.md` | 🔴 |
| AC2 | **Activar plan de promoción** de los 3 productos (mínimo viable: 4 piezas LinkedIn + 1 secuencia email + 1 colaboración) | 🔴 |
| AC3 | Definir plataforma de cursos (Hotmart · Teachable · Kajabi · LATAM) | 🟡 |
| AC4 | Confirmar plataforma de tienda actual + fulfillment | 🟡 |
| AC5 | Identidad visual (logo, paleta, tipografías) | 🟡 |
| AC6 | Catálogo MVP de cursos: 1 curso + 1 bundle plantillas para beta | 🟡 |
| AC7 | Web propia consolidada (dominio, landing) | 🟡 |
| AC8 | Política de licencia: cómo ARP usa el Marco ACA™ de Sonia personal | 🟡 |

---

## Reglas Academia ARP

1. Todo contenido educativo cita el Marco ACA™ y los DOIs Zenodo cuando aplique.
2. No vende auditorías 1-a-1 — eso es Blum.
3. Tier claro: ARP es self-service, escalable, masivo. Blum es premium, custom, elite.
4. Merch nunca usa el Marco ACA™ como producto principal — solo como elemento gráfico.
5. Sonja Kalos no aparece en ningún canal de Academia ARP.
