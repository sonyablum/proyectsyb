# 📚 Dossier académico — Sonia Yánez Blum

> Fuente canónica para `agente-congresos-academicos`. Toda referencia a publicaciones, DOIs, congresos y pipeline doctoral debe leer de aquí.

**ORCID**: [0000-0002-6695-8129](https://orcid.org/0000-0002-6695-8129)
**Programa doctoral**: HDA — Humans in the Digital Age (AAU Klagenfurt)
**Supervisora**: Dra. Valerie Hase
**Sede**: Klagenfurt, Austria → trabajo de campo LATAM
**Defensa proyectada**: Q3 2028

---

## ⚠️ Reglas críticas de comunicación pública

| Regla | Razón |
|---|---|
| Decir **"investigadora independiente en reputación algorítmica"** | NUNCA "doctoranda", "candidata doctoral", "mi doctorado" |
| Citar SOLO **ACA-Score™ v2 (DOI Zenodo 19035939)** públicamente | v3 es borrador interno sin DOI; v3.2 es trabajo |
| NO mencionar Amcham como caso de estudio académico | Conflicto comercial / reputacional |
| NO mencionar campañas políticas como casos | Mismo motivo |
| Idioma según sede | Hase → inglés. CINCOMA/MEDIAETHICS → español |

---

## 🎯 Pregunta de investigación canónica

**Versión EN (para Hase y publicaciones internacionales)**:
> "Under what structural conditions does an entity become visible and characterised as a thematic authority in LLM-based systems — and under what conditions does it cease to be so — with consequences for what reaches news media, citizen queries, and public discourse?"

**Versión ES (publicaciones LATAM/España)**:
> ¿Bajo qué condiciones estructurales una entidad se vuelve visible y caracterizada como autoridad temática en sistemas basados en LLMs (ChatGPT, Claude, Gemini, Perplexity), y bajo qué condiciones deja de serlo — con consecuencias para lo que llega a medios, ciudadanos y discurso público?

---

## 📑 Publicaciones canónicas (Zenodo preprints)

| ID | Título | Fecha | DOI | Cita |
|---|---|---|---|---|
| **TAA** | Gestión de la reputación en la era de los buscadores con IA: aplicación exploratoria de la Teoría de la Autoridad Algorítmica | Ene–Feb 2026 | [10.5281/zenodo.18802347](https://doi.org/10.5281/zenodo.18802347) | Yánez Blum, 2026a |
| **ACA-Score™ v2** ★ | Instrumento metodológico — 4 pilares · 12 KPIs · fórmula canónica | 15 mar 2026 | [10.5281/zenodo.19035939](https://doi.org/10.5281/zenodo.19035939) | Yánez Blum, 2026b |
| **RICFE® v2.4** | Framework de prompting estratégico — 5 componentes · n=123 | Abr 2026 | [10.5281/zenodo.19388591](https://doi.org/10.5281/zenodo.19388591) | Yánez Blum, 2026c |

### Documentos en preparación (sin DOI todavía)
- **ACA-Score™ v3** — Marco Matemático Bidimensional (borrador 26-abr-2026). ⚠️ NO citar públicamente. Pendiente depositar en Zenodo.
- **CINCOMA 2025 ponencia** — pendiente depositar en Zenodo para DOI (citar como Yánez Blum, 2025a hasta entonces).

---

## 🏛 Pipeline de congresos

### ✅ Aprobados 2026

| Congreso | Fecha | Sede | Estado | Pendiente |
|---|---|---|---|---|
| **AIEthics 2026** (UCM/Intracom/US) | 16 mar 2026 | Madrid | Presentado ✓ | 🔴 **Pago inscripción BBVA pendiente** |
| **Congreso H 2026** | 27–30 may 2026 | Online | Abstract aprobado | Ponencia completa antes 27-may |
| **MEDIAETHICS 2026** | 29–30 jun online / 1–3 jul presencial | Valladolid | Abstract aprobado | Ponencia + 🔴 **Pago inscripción BBVA** |

### Datos de pago
- BBVA: `ES5301826101850201610161`
- PayPal: `activa19comunicaciones@gmail.com`
- Contacto Intracom: Aránzazu Román `arantxa@us.es`

### Publicaciones derivadas comprometidas
- Monográfico **Tirant/Dykinson** (peer review, SPI) — derivado AIEthics
- **Intracom Journal** junio 2026 — derivado AIEthics

---

## 🧪 P3 Protocol — Fase empírica en curso

**Diseño**: 50 queries × 4 LLMs × 3 iteraciones × 2 idiomas = 600 datos por sondeo.
**LLMs**: ChatGPT · Gemini · Perplexity · Claude.
**Distribución**: por los 4 pilares ACA™.

| Sondeo | Fechas | Estado |
|---|---|---|
| **S1 — Baseline** | 22–25 abr 2026 | ⚠️ Verificar que esté completo |
| **S2 — Follow-up longitudinal** (+10 días exactos) | ~5 may 2026 | 🔴 **Próximo** |
| Análisis comparativo S1↔S2 | May 2026 | Output: estabilidad temporal, variabilidad inter-plataforma, sensibilidad por pilar, efecto idioma |

---

## 📊 Delphi modificado — Validación del constructo

| Ronda | n | Estado | Hallazgo clave |
|---|---|---|---|
| Exploratoria (R0) | 130 | Ago–Sep 2025 ✓ | 7 países LATAM. Riesgos: errores/falsedades, falta verificación → **Pilar Trazabilidad** |
| Piloto estructurado | 5 | Ene–Feb 2026 ✓ | Todos usan IA, mayoría sin política formal |
| **R1 modificado** | 18 | Abr 2026 ✓ | 61.1% +15 años exp · 77.8% maestría/doctorado · brecha auditabilidad: importancia 88.9% (M=4.50) vs factibilidad 55.6% (M=3.78) |
| **R2** | 18 | 🔴 **Cierre 29-abr-2026 — verificar** | Integrar en paper Congreso H + MEDIAETHICS |

---

## 🗓 Plan de validación 2026–2028 (6 estudios — ACA-Score™ v3)

| ID | Estudio | Cuándo | Output objetivo |
|---|---|---|---|
| E1 | AFC del ACA-C (WLSMV, N≥300, CFI≥0.95, RMSEA≤0.06) | Q4 2026 | Estructura 4 pilares confirmada |
| E2 | P3 observacional (100 entidades × 600 obs = 60.000 datos) | Q4 2026 | GLMM, ICC inter-plataforma, α-Krippendorff |
| E3 | Inter-jueces CA y RE (3 jueces, 600 menciones) | Q1–Q2 2027 | α-Krippendorff ≥0.75 |
| E4 | Experimento RICFE® vs prompting libre (N=160, ANCOVA) | Q1–Q2 2027 | Cohen's d IC95% |
| E5 | SEM unificado RICFE_quality → ACA-C → ACA-A (N≥400, bootstrap 5k) | Q3–Q4 2027 | Validez predictiva |
| E6 | Replicación intercultural (LATAM + España + Austria/Alemania, N=200×3) | Q3–Q4 2027 | Invariancia de medición ΔCFI≤0.01 |

**Pre-registro OSF** antes de cada recolección.

**Revistas objetivo**: Public Relations Review · Journal of Communication · New Media & Society.

---

## 🤝 Conexión académica con Hase / WEAVE

**Proyecto WEAVE de Hase**: diversidad de representaciones digitales en noticias.
**Conexión con ACA**: mismo objeto teórico (diversidad de representaciones digitales), objeto nuevo = **actores organizacionales** en lugar de noticias.
**Paper relevante**: Hase 2024 con TeBlunthuis & Chan → directamente aplicable para construir gold standard del P3 Protocol.

---

## 📦 Archivos fuente

| Archivo | Ubicación | Contenido |
|---|---|---|
| `cronologia-syab-2024-2028.html` | este directorio | Versión visual entregada el 30-abr-2026 |
| `dossier-academico.md` | este archivo | Fuente canónica editable |
| `marco-aca/` | repo raíz | Documentos conceptuales (pendientes subir v3 + IA-Ethics) |

---

## 🔄 Mantenimiento

Actualizar este dossier cuando:
- Se publique un DOI nuevo en Zenodo → añadir a tabla de publicaciones.
- Se acepte/rechace abstract en congreso → mover entre tablas.
- Cambien fechas de Sondeos P3.
- Se cierre una ronda Delphi.
- Hase confirme cualquier cambio de programa o supervisión.
